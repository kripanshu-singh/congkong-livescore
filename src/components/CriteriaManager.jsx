...
import { Plus, Trash2, Save, AlertCircle, CheckCircle2, GripVertical, Layers, Hash, Type, HelpCircle } from 'lucide-react';
...
export const CriteriaManager = ({ settings, onSave }) => {
  ...
  const initializeState = () => {
    const defaultCats = [
       { id: 'cat_creativity', label: '창의성', label_en: 'Creativity', max: 30 },
  ...
  const [activeTab, setActiveTab] = useState(categories[0]?.id || 'cat_creativity');
  ...
  const handleSave = () => {
    ...
    categories.forEach(cat => {
      const sum = items.filter(i => i.category === cat.id).reduce((a, b) => a + b.max, 0);
      if (sum !== cat.max) {
    ...
    <div className="h-full flex flex-col gap-6 p-1">
       ...
       <div className="flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3">
             ...
             <div>
               <h2 className="text-xl font-bold text-slate-800">{t.criteria_title}</h2>
       ...
       <div className="flex-1 flex flex-col lg:flex-row gap-6 min-h-0">
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-4 overflow-hidden">
           ...
           <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
             {categories.map(cat => {
                const catSum = items.filter(i => i.category === cat.id).reduce((a, b) => a + b.max, 0);
                   ...
                   >
                      <div className="flex justify-between items-start mb-3 relative z-10">
                         <div className="flex-1">
                            ...
                            <input 
                               value={cat.label_en || ''}
                               onChange={(e) => handleUpdateCategory(cat.id, 'label_en', e.target.value)}
                      ...
                      {isError && (
                         <div className="mt-2 flex items-center gap-1 text-[10px] font-bold text-red-500 animate-pulse relative z-10">
                            <AlertCircle className="w-3 h-3" /> {t.item_sum_error.replace('{sum}', catSum).replace('{max}', cat.max)}
                         </div>
                      )}
          ...
          {activeCategory ? (
              <GlassCard className="flex-1 flex flex-col p-0 overflow-hidden border-slate-200 shadow-xl bg-white/80">
                  ...
                  <div className="p-5 border-b border-slate-100 bg-white sticky top-0 z-20 flex justify-between items-center shrink-0">
                      <div>
                          <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                             {activeCategory.label || <span className="text-slate-400 italic">Untitled Category</span>}
                             <span className="text-sm font-normal text-slate-400">({activeCategory.label_en})</span>
                          </h3>
                          <p className="text-xs text-slate-500 mt-1">{t.criteria_desc}</p>
                      </div>
                      <div className="flex gap-2">
                         <div className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 border ${currentSum === activeCategory.max ? 'bg-green-50 text-green-600 border-green-200' : 'bg-red-50 text-red-600 border-red-200'}`}>
                             {currentSum === activeCategory.max ? <CheckCircle2 className="w-4 h-4"/> : <AlertCircle className="w-4 h-4"/>}
                  ...
                  <div className="grid grid-cols-12 gap-3 px-4 py-2 bg-slate-50 border-b border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-wider shrink-0">
                      ...
                      <div className="col-span-3 flex items-center gap-1"><Type className="w-3 h-3"/> {t.item_name_ko}</div>
                      <div className="col-span-3 flex items-center gap-1"><Type className="w-3 h-3"/> {t.item_name_en}</div>
                      <div className="col-span-2 flex items-center gap-1"><Hash className="w-3 h-3"/> {t.max_score}</div>
                      <div className="col-span-2 flex items-center gap-1"><HelpCircle className="w-3 h-3"/> {t.item_desc}</div>
                      ...
                      <button 
                         onClick={() => {
                             if(window.confirm(t.confirm_delete_category)) {
                                 setCategories(categories.filter(c => c.id !== activeTab));
