...
import { AlertTriangle, X, AlertCircle, ShieldCheck, Lock, Key, Eye, EyeOff } from 'lucide-react';
import { AppContext } from '../context';

// Submit Confirmation Modal
export const ConfirmSubmitModal = ({ isOpen, onClose, onConfirm, totalScore, zeroItems }) => {
  const { t, lang } = useContext(AppContext);
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-[#1c1c1e] w-[90%] max-w-[400px] rounded-[32px] p-8 shadow-2xl border border-white/10 animate-in zoom-in-95">
        <h3 className="text-2xl font-bold mb-2 text-center text-slate-900 dark:text-white">{t.confirm_title}</h3>
        
        <div className="my-6 text-center">
          <div className="text-sm text-slate-500 mb-1">{t.confirm_msg}</div>
          <div className="text-6xl font-black text-blue-600 dark:text-blue-400 tracking-tighter">{totalScore}</div>
        ...
        {zeroItems.length > 0 && (
          <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-xl border border-red-100 dark:border-red-900/50 mb-6">
            ...
            <div className="text-xs text-slate-600 dark:text-slate-300">
              {zeroItems.map(item => lang === 'en' ? item.label_en : item.label).join(', ')} {t.confirm_warn_desc}
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={onClose} className="py-3.5 rounded-xl font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 transition-colors text-sm">{t.btn_recheck}</button>
...
export const TeamDetailModal = ({ isOpen, onClose, team, judges, scores }) => {
  ...
  const submittedScores = teamScores.filter(t => t.score).map(t => t.score.total);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in" onClick={onClose}>
      <div className="bg-white dark:bg-[#1c1c1e] w-[90%] max-w-[600px] rounded-[32px] p-8 shadow-2xl border border-white/10 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
          {teamScores.map(({judge, score}) => {
            const isMax = score && score.total === maxScore && submittedScores.length > 2;
            const isMin = score && score.total === minScore && submittedScores.length > 2;
            const jName = lang === 'en' ? judge.name_en : judge.name;
            
            return (
              <div key={judge.id} className={`flex items-center justify-between p-4 rounded-xl border ${score ? 'bg-slate-50 dark:bg-white/5 border-slate-100 dark:border-white/5' : 'bg-red-50 dark:bg-red-900/10 border-red-100 dark:border-red-900/30'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${score ? 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300' : 'bg-red-100 text-red-600'}`}>
                    {jName[0]}
                  </div>
                  <div>
                    <div className={`font-bold text-sm ${!score && 'text-red-600 dark:text-red-400'}`}>{jName}</div>
                    <div className="text-xs text-slate-400">{judge.company}</div>
                  </div>
                </div>
                ...
                {score ? (
                  <div className="text-right">
                    <div className="flex items-center justify-end gap-2">
                       ...
                       <div className={`text-2xl font-black ${isMax || isMin ? 'text-slate-400 line-through decoration-slate-400/50' : 'text-slate-800 dark:text-white'}`}>{score.total}</div>
                    </div>
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 dark:border-white/10 flex justify-between text-xs text-slate-400">
           <span>{t.detail_total_judges}: {judges.length}</span>
           <span className={teamScores.filter(t => t.score).length < judges.length ? 'text-red-500 font-bold' : 'text-green-500 font-bold'}>
             {t.detail_completed}: {teamScores.filter(t => t.score).length}/{judges.length}
           </span>
        </div>
      </div>
    </div>
...
export const AdminLoginModal = ({ isOpen, onClose, onLogin }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-[#1c1c1e] w-[360px] rounded-[32px] p-8 shadow-2xl border border-white/10 animate-in zoom-in-95">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
           <div className="relative">
              <div className="absolute left-4 top-3.5 text-slate-400"><Lock className="w-4 h-4" /></div>
              <input 
                type="text" placeholder={t.admin_placeholder_id} autoFocus
                value={id} onChange={(e) => setId(e.target.value)}
                className="w-full bg-slate-100 dark:bg-black/50 rounded-2xl py-3 pl-12 pr-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
              />
           </div>
           <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-slate-100 dark:bg-black/50 rounded-2xl py-3 pl-12 pr-12 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
              />
...
export const SignatureModal = ({ isOpen, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-md animate-in fade-in">
      <div className="bg-white dark:bg-[#1c1c1e] w-[90%] max-w-[400px] rounded-[32px] p-6 shadow-2xl animate-in zoom-in-95">
        ...
        <div className="bg-slate-50 dark:bg-black/30 rounded-2xl border border-slate-200 dark:border-slate-700 h-40 w-full relative overflow-hidden mb-4">
          <canvas
            ref={canvasRef}
