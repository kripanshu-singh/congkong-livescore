      ...
      <div className="bg-white  w-[90%] max-w-[600px] rounded-[32px] p-8 shadow-2xl border border-white/10 animate-in zoom-in-95 cursor-pointer" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
          {teamScores.map(({judge, score}) => {
            ...
            return (
              <div key={judge.id} className={`flex items-center justify-between p-4 rounded-xl border ${score ? 'bg-slate-50  border-slate-100 ' : 'bg-red-50  border-red-100 '}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${score ? 'bg-slate-200  text-slate-600 ' : 'bg-red-100 text-red-600'}`}>
                    {jName[0]}
                  </div>
                  <div>
                    <div className={`font-bold text-sm ${!score && 'text-red-600 '}`}>{jName}</div>
