...
export default function App() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        // Try to enable persistence (local storage)
  ...
  const handleSubmitScore = async (teamId, detail, comment, signature) => {
    if (!user) {
      alert(
        "Error: Not connected to the server. Please ensure 'Anonymous Authentication' is enabled in your Firebase Console.",
      );
